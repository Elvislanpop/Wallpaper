...
typedef NS_ENUM(NSUInteger, HYDCachePolicy){
    /**只从网络获取数据，且数据不会缓存在本地*/
    HYDCachePolicyIgnoreCache = 0,
    /**只从缓存读数据，如果缓存没有数据，返回一个空*/
    HYDCachePolicyCacheOnly = 1,
    /**先从网络获取数据，同时会在本地缓存数据*/
    HYDCachePolicyNetworkOnly = 2,
    /**先从缓存读取数据，如果没有再从网络获取*/
    HYDCachePolicyCacheElseNetwork = 3,
    /**先从网络获取数据，如果没有在从缓存获取，此处的没有可以理解为访问网络失败，再从缓存读取*/
    HYDCachePolicyNetworkElseCache = 4,
    /**先从缓存读取数据，然后在从网络获取并且缓存，在这种情况下，Block将产生两次调用*/
    HYDCachePolicyCacheThenNetwork = 5
};

/**请求方式*/
typedef NS_ENUM(NSUInteger, HYDRequestMethod){
    /**GET请求方式*/
    HYDRequestMethodGET = 0,
    /**POST请求方式*/
    HYDRequestMethodPOST,
    /**HEAD请求方式*/
    HYDRequestMethodHEAD,
    /**PUT请求方式*/
    HYDRequestMethodPUT,
    /**PATCH请求方式*/
    HYDRequestMethodPATCH,
    /**DELETE请求方式*/
    HYDRequestMethodDELETE
};

typedef NS_ENUM(NSUInteger, HYDNetworkStatusType){
    /**未知网络*/
    HYDNetworkStatusUnknown,
    /**无网路*/
    HYDNetworkStatusNotReachable,
    /**手机网络*/
    HYDNetworkStatusReachableWWAN,
    /**WiFi网络*/
    HYDNetworkStatusReachableWiFi
};

typedef NS_ENUM(NSUInteger, HYDRequestSerializer){
    /**设置请求数据为JSON格式*/
    HYDRequestSerializerJSON,
    /**设置请求数据为二进制格式*/
    HYDRequestSerializerHTTP
};

typedef NS_ENUM(NSUInteger, HYDResponseSerializer) {
    /**设置响应数据为JSON格式*/
    HYDResponsetSerializerJSON,
    /**设置响应数据为二进制格式*/
    HYDResponseSerializerHTTP
};

/**请求的Block*/
typedef void(^HYDHttpRequest)(id responseObject, NSError *error);

/**下载的Block*/
typedef void(^HYDHttpDownload)(NSString *path, NSError *error);

/*上传或者下载的进度*/
typedef void(^HYDHttpProgress)(NSProgress *progress);

/**网络状态Block*/
typedef void(^HYDNetworkStatus)(HYDNetworkStatusType status);



@interface HYDNetwork : NSObject
...
+ (void)uploadFileWithURL:(NSString *)url
               parameters:(NSDictionary *)parameters
                 ...
                 progress:(HYDHttpProgress)progress
                  callback:(HYDHttpRequest)callback;
...
+ (void)downloadWithURL:(NSString *)url
               ...
               progress:(HYDHttpProgress)progress
                callback:(HYDHttpDownload)callback;
...
/**
 ...
 @param requestSerializer HYDRequestSerializerJSON---JSON格式  HYDRequestSerializerHTTP--HTTP
 */
...
/**
 ...
 @param responseSerializer HYDResponseSerializerJSON---JSON格式  HYDResponseSerializerHTTP--HTTP
